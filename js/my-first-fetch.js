"use strict"

// let lat = 33.44;
// let lon = -94.04;
// let units = "imperial";
//
//
// //do fetch stuff
// fetch("https://api.openweathermap.org/data/2.5/onecall?lat="+ lat + "&lon=-94.04&units=imperial&appid=" + OWM_KEY)
//     .then(response => response.json())
//     .then(data => {
//         console.log(data)
//         let html = "";
//         html += '<ul>'
//         // for (let i = 0; i < data.daily.length; i++) {
//         //     console.log(data.daily[i].temp.day);
//         // }
//
//         for (let i = 0; i < 5; i++) {
//             let dailyTemp = data.daily[i].temp.day
//
//             html += '<li>' + dailyTemp + '</li>';
//         }
//
//         html += '</ul>';
//
//         $('#weather').html(html);
//     });


// fetch("https://api.openweathermap.org/data/2.5/onecall?lat="+ lat + "&lon=-94.04&units=imperial&appid=asdfklj3443523lkjsfdlgkjdsfg4554645lkjdhlkdfhj", options)
//     .then(response => response.json())
//     .then(response => getSanitizedResponse(response))
//     .then(sres => renderDailyForecast(sres))
//     .catch(err => console.error(err));

// function getSanitizedResponse(dataBody) {
//     return dataBody.main;
// }
//
// //language=HTML
// function renderDailyForecast(forecast) {
//     console.log(forecast);
//     $('body').append(`
//         <div><h3>Temperature Feels Like:</h3><p>${forecast.feels_like}</p></div>`);
// }

$(document).ready(function () {

    $('body').prepend("<section id=box-one></section>");

    /*DO FETCH STUFF*/
    fetch("https://swapi.dev/api/planets/")
        .then(response => response.json())
        .then(data => popBody(data))
        .catch(err => console.error(err));

    //language=HTML
    function popBody(dataIn) {

        console.log(dataIn);

        //iterate through api and create new property per object + return modified array of objects (ea. object = planet now) as mappedArr
        let mappedArr = dataIn.results.map((planet) => {
            if (planet.name.toLowerCase() === 'tatooine') {
                planet.backgroundImg = "data:image/jpeg;base64,";
            } else if (planet.name.toLowerCase() === 'alderaan') {
                planet.backgroundImg ="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/d71e379b-3f09-42b2-b3fe-26548591a750/dev17yq-ce816241-f6aa-444a-94b6-7104f945e050.jpg/v1/fill/w_894,h_894,q_70,strp/alderaan_star_wars_planet_collection_by_ericwhitted_dev17yq-pre.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..W6cfuwMa6rG_ikKFamBc53AdSgUg3aCmD3wTrSQm-uY"
            } else if (planet.name.toLowerCase() === 'yavin iv') {
                planet.backgroundImg ="https://lumiere-a.akamaihd.net/v1/images/databank_yavin4_01_169_b6945e20.jpeg?region=0%2C49%2C1560%2C780"
            } else if (planet.name.toLowerCase() === 'hoth') {
                planet.backgroundImg ="https://assets.vg247.com/current//2015/11/star_wars_battlefront_hoth-1.jpg"
            } else if (planet.name.toLowerCase() === 'dagobah') {
                planet.backgroundImg ="https://lumiere-a.akamaihd.net/v1/images/Dagobah_890df592.jpeg?region=0%2C80%2C1260%2C630"
            } else if (planet.name.toLowerCase() === 'bespin') {
                planet.backgroundImg ="https://lumiere-a.akamaihd.net/v1/images/Bespin_2d0759aa.jpeg?region=0%2C0%2C1560%2C878&width=960"
            } else if (planet.name.toLowerCase() === 'endor') {
                planet.backgroundImg ="https://www.hollywoodreporter.com/wp-content/uploads/2015/11/the_planets_of_star_wars_battlefront_teaser_grab.jpg"
            } else if (planet.name.toLowerCase() === 'naboo') {
                planet.backgroundImg ="https://i0.wp.com/overmental.com/wp-content/uploads/2015/07/Naboo-TPM.jpg?fit=1920%2C816&ssl=1"
            } else if (planet.name.toLowerCase() === 'coruscant') {
                planet.backgroundImg ="https://qph.fs.quoracdn.net/main-qimg-c1f1373d375c6e941d941184a873fb35"
            } else if (planet.name.toLowerCase() === 'kamino') {
                planet.backgroundImg ="https://1.bp.blogspot.com/-kEomXtUqO6Y/Wcrj01hjiQI/AAAAAAAA4WE/eZED7HpZIvUVYq0FySNT-WO664qkiiQDgCLcBGAs/s1600/mapvista-kamino.jpg"
            }
                return planet;
        });

        //log new object array mappedArr
        console.log(mappedArr);

        //create selector assigned to variable boxHTMLOne
        let boxHtmlOne = $('<div class="row justify-content-between"></div>');

        //iterate through each planet object and create cards
        mappedArr.forEach((planet) => {
            let pNameOne = planet.name;
            let pClimateOne = planet.climate;
            let popOne = planet.population;

            boxHtmlOne.append(`
                <div class="col-4">
                    <p class="d-flex justify-content-center align-items-center" style="background-image: url(${planet.backgroundImg}); background-repeat: no-repeat; background-position: center; background-size: 100% 300px"><strong>${pNameOne}</strong></p>
                    <ul class="d-flex justify-content-center align-items-center">
                        <li><strong>Climate: ${pClimateOne}<strong></li>
                        <li><strong>Population: ${popOne} <strong></li>
                    </ul>
                </div>`)
        });
        //
        $('#box-one').html(boxHtmlOne);
    }
});